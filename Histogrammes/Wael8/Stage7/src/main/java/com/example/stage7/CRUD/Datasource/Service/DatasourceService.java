...
/*
    ...
    public String buildElasticsearchUrl(String idDatasource) {
        ...
        if (optionalDatasource.isPresent()) {
            ...
            Chart chart = datasource.getChart();
            if (chart != null) {
                return "http://" + datasource.getUser() + ":" + datasource.getConnection_port() + "/" + chart.getIndex() + "/_search";
            }
        }
        return null; // Gérer le cas où le datasource ou le chart est introuvable
    }
*/
    @Override
// Méthode pour construire l'URL Elasticsearch
    public String getElasticsearchUrl(String idDatasource) {
        Optional<Datasource> optionalDatasource = datasourceRepository.findById(idDatasource);
        if (optionalDatasource.isPresent()) {
            Datasource datasource = optionalDatasource.get();
            Chart chart = datasource.getChart();
